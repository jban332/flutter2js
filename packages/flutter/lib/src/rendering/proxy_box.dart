...
/// of their child.
class RenderProxyBox extends RenderBoxWithChildMixin<RenderBox> with RenderProxyBoxMixin {
...
class RenderAspectRatio extends RenderProxyBox {
  ...
  void performLayout() {
    ...
    if (child != null) child.layout(new BoxConstraints.tight(size));
  }
...
class RenderAnimatedOpacity extends RenderProxyBox {
  ...
  RenderAnimatedOpacity({@required Animation<double> opacity, RenderBox child})
      : super(child) {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder description) {
    super.debugFillProperties(description);
    description
        .add(new DiagnosticsProperty<Animation<double>>('opacity', opacity));
  }
}
...
class RenderBackdropFilter extends RenderProxyBox {
  ...
  void paint(PaintingContext context, Offset offset) {
    if (child != null) {
      assert(needsCompositing);
      context.pushLayer(
          new BackdropFilterLayer(filter: _filter), super.paint, offset);
    }
  }
}
...
class ShapeBorderClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    return shapeBorder.getOuterPath(Offset.zero & size,
...
abstract class _RenderCustomClip<T> extends RenderProxyBox {
  _RenderCustomClip({RenderBox child, CustomClipper<T> clipper})
      : _clipper = clipper,
        super(child);
    ...
    assert(() {
      if (child != null) {
        ...
        _debugText.paint(
            ...
            offset +
                new Offset(
                    _clip.tlRadiusX, -_debugText.text.style.fontSize * 1.1));
      }
      return true;
    }());
  }
    ...
    assert(() {
      if (child != null) {
        ...
        _debugText.paint(
            context.canvas,
            offset +
                new Offset((_clip.width - _debugText.width) / 2.0,
...
class RenderClipPath extends _RenderCustomClip<Path> {
  ...
  /// efficiently.
  RenderClipPath({RenderBox child, CustomClipper<Path> clipper})
      : super(child: child, clipper: clipper);
...
abstract class _RenderPhysicalModelBase<T> extends _RenderCustomClip<T> {
  ...
  static final Paint _transparentPaint = new Paint()
    ...
    if (child != null) {
      ...
      if (needsCompositing) {
        ...
        if (!offsetClipRRect.isRect)
          canvas.saveLayer(
              offsetBounds, _RenderPhysicalModelBase._defaultPaint);
        super.paint(context, offset);
        if (!offsetClipRRect.isRect) canvas.restore();
        canvas.restore();
...
class RenderPhysicalShape extends _RenderPhysicalModelBase<Path> {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder description) {
    super.debugFillProperties(description);
    description
        .add(new DiagnosticsProperty<CustomClipper<Path>>('clipper', clipper));
  }
}
    ...
    if (position == DecorationPosition.background) {
      ...
      assert(() {
        if (debugSaveCount != context.canvas.getSaveCount()) {
          throw new FlutterError('${_decoration
                  .runtimeType} painter had mismatching save and restore calls.\n'
              'Before painting the decoration, the canvas save count was $debugSaveCount. '
              'After painting it, the canvas save count was ${context.canvas
                  .getSaveCount()}. '
...
class RenderTransform extends RenderProxyBox {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder description) {
    ...
    description.add(new EnumProperty<TextDirection>(
        ...
        defaultValue: null));
    description.add(
        new DiagnosticsProperty<bool>('transformHitTests', transformHitTests));
  }
}
  ...
  void _updatePaintData() {
    ...
    if (child == null) {
      ...
      _hasVisualOverflow = sourceRect.width < childSize.width ||
          sourceRect.height < childSize.width;
      _transform = new Matrix4.translationValues(
...
class RenderFractionalTranslation extends RenderProxyBox {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder description) {
    super.debugFillProperties(description);
    description
        .add(new DiagnosticsProperty<Offset>('translation', translation));
    description.add(
        new DiagnosticsProperty<bool>('transformHitTests', transformHitTests));
  }
}
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder description) {
    ...
    assert(() {
      ...
      if (debugSymmetricPaintCount + debugAsymmetricPaintCount == 0) {
        description.add(new MessageProperty(
        ...
        description.add(new PercentProperty('metrics', fraction,
            unit: 'useful',
            tooltip:
    ...
    if (inReleaseMode)
      description.add(new DiagnosticsNode.message(
...
class RenderAbsorbPointer extends RenderProxyBox {
  ...
  RenderAbsorbPointer({RenderBox child, this.absorbing: true}) : super(child);
  ...
  void _performSemanticScrollLeft() {
    if (onHorizontalDragUpdate != null) {
      ...
      onHorizontalDragUpdate(new DragUpdateDetails(
        delta: new Offset(primaryDelta, 0.0),
  ...
  void _performSemanticScrollUp() {
    if (onVerticalDragUpdate != null) {
      ...
      onVerticalDragUpdate(new DragUpdateDetails(
        delta: new Offset(0.0, primaryDelta),
        primaryDelta: primaryDelta,
        globalPosition: localToGlobal(size.center(Offset.zero)),
      ));
    }
...
class RenderLeaderLayer extends RenderProxyBox {
  ...
  void paint(PaintingContext context, Offset offset) {
    context.pushLayer(
        new LeaderLayer(link: link, offset: offset), super.paint, Offset.zero);
  }
...
class RenderFollowerLayer extends RenderProxyBox {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder description) {
    ...
    description.add(new DiagnosticsProperty<LayerLink>('link', link));
    description.add(
        new DiagnosticsProperty<bool>('showWhenUnlinked', showWhenUnlinked));
    description.add(new DiagnosticsProperty<Offset>('offset', offset));
    description.add(new TransformProperty(
        'current transform matrix', getCurrentTransform()));
  }
}
