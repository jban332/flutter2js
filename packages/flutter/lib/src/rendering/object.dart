...
class PaintingContext {
  ...
  void pushLayer(Layer childLayer, PaintingContextCallback painter,
      Offset offset,
    ...
    final PaintingContext childContext =
    new PaintingContext._(childLayer, childPaintBounds ?? canvasBounds);
  ...
  void pushClipRect(bool needsCompositing, Offset offset, Rect clipRect,
    ...
    if (needsCompositing) {
      pushLayer(new ClipRectLayer(clipRect: offsetClipRect), painter, offset,
  ...
  void pushClipRRect(bool needsCompositing, Offset offset, Rect bounds,
    ...
    if (needsCompositing) {
      pushLayer(new ClipRRectLayer(clipRRect: offsetClipRRect), painter, offset,
  ...
  void pushClipPath(bool needsCompositing, Offset offset, Rect bounds,
    ...
    if (needsCompositing) {
      pushLayer(new ClipPathLayer(clipPath: offsetClipPath), painter, offset,
          childPaintBounds: offsetBounds);
    } else {
      canvas
        ..save()
  ...
  void pushTransform(bool needsCompositing, Offset offset, Matrix4 transform,
    ...
    if (needsCompositing) {
      pushLayer(
        ...
        offset,
        childPaintBounds:
        MatrixUtils.inverseTransformRect(effectiveTransform, canvasBounds),
      );
    } else {
      canvas
        ..save()
  ...
  @override
  String toString() =>
      '$runtimeType#$hashCode(layer: $_containerLayer, canvas bounds: $canvasBounds)';
}
...
class _SemanticsGeometry {
  ...
  Rect _intersectClipRect(Rect other) {
    if (_clipRect == null) return other;
    if (other == null) return _clipRect;
  ...
  void applyAncestorChain(List<RenderObject> ancestorChain) {
    for (int index = ancestorChain.length - 1; index > 0; index -= 1) {
      ...
      _clipRect =
          _intersectClipRect(parent.describeApproximatePaintClip(child));
      if (_clipRect != null) {
        if (_clipRect.isEmpty) {
          ...
          _clipRect =
              MatrixUtils.inverseTransformRect(clipTransform, _clipRect);
        }
      }
...
abstract class _SemanticsFragment {
  ...
  Iterable<SemanticsNode> compile({_SemanticsGeometry geometry,
...
class _EmptySemanticsFragment extends _SemanticsFragment {
  ...
  Iterable<SemanticsNode> compile({_SemanticsGeometry geometry,
    SemanticsNode currentSemantics,
    SemanticsNode parentSemantics}) sync* {}
...
class _CleanSemanticsFragment extends _SemanticsFragment {
  ...
  Iterable<SemanticsNode> compile({_SemanticsGeometry geometry,
    ...
    if (geometry != null) {
      ...
      geometry.updateSemanticsNode(
          rendering: renderObjectOwner,
          semantics: node,
...
abstract class _InterestingSemanticsFragment extends _SemanticsFragment {
  ...
  Iterable<SemanticsNode> compile({_SemanticsGeometry geometry,
    ...
    establishSemanticsNode(geometry, currentSemantics, parentSemantics);
  ...
  SemanticsNode establishSemanticsNode(_SemanticsGeometry geometry,
      SemanticsNode currentSemantics, SemanticsNode parentSemantics);

  Iterable<SemanticsNode> finalizeSemanticsNode(SemanticsNode node,
      List<SemanticsNode> children);

  _SemanticsGeometry createSemanticsGeometryForChild(
...
class _RootSemanticsFragment extends _InterestingSemanticsFragment {
  ...
  SemanticsNode establishSemanticsNode(_SemanticsGeometry geometry,
    ...
    renderObjectOwner._semantics ??= new SemanticsNode.root(
      handler: renderObjectOwner is SemanticsActionHandler
          ? renderObjectOwner as dynamic
          : null,
  ...
  @override
  Iterable<SemanticsNode> finalizeSemanticsNode(SemanticsNode node,
      List<SemanticsNode> children) sync* {
    if (annotator != null) annotator(node);
...
class _ForkingSemanticsFragment extends _SemanticsFragment {
  ...
  @override
  Iterable<SemanticsNode> compile({@required _SemanticsGeometry geometry,
    SemanticsNode currentSemantics,
    SemanticsNode parentSemantics}) sync* {
    assert(!_debugCompiled);
    assert(() {
...
class SemanticsHandle {
  ...
  void dispose() {
    assert(() {
      if (_owner == null) {
        throw new FlutterError('SemanticsHandle has already been disposed.\n'
...
abstract class RenderObject extends AbstractNode
  ...
  void _debugReportException(String method, dynamic exception,
      StackTrace stack) {
  ...
  void layout(Constraints constraints, {bool parentUsesSize: false}) {
    ...
    assert(constraints.debugAssertIsValid(
        ...
        informationCollector: (StringBuffer information) {
          ...
          new RegExp(r'^#[0-9]+ +RenderObject.layout \(');
  ...
  void _paintWithContext(PaintingContext context, Offset offset) {
    assert(() {
      if (_debugDoingThisPaint) {
        throw new FlutterError('Tried to paint a RenderObject reentrantly.\n'
  ...
  void markNeedsSemanticsUpdate(
      {bool onlyLocalUpdates: false, bool noGeometry: false}) {
    assert(!attached || !owner._debugDoingSemantics);
    if ((attached && owner._semanticsOwner == null) ||
        (_needsSemanticsUpdate &&
            onlyLocalUpdates &&
            (_needsSemanticsGeometryUpdate || noGeometry))) return;
    if (!noGeometry &&
        (_semantics == null ||
            (_semantics.hasChildren && _semantics.wasAffectedByClip))) {
    ...
    if (onlyLocalUpdates) {
      ...
      while (node._semantics == null && node.parent is RenderObject) {
        if (node._needsSemanticsUpdate) return;
      ...
      RenderObject node = this;
      do {
        if (node.parent is! RenderObject) break;
        node._needsSemanticsUpdate = true;
        node.resetSemantics();
        node = node.parent;
  ...
  void _updateSemantics() {
    try {
      ...
      final SemanticsNode node =
          fragment
              .compile(parentSemantics: _semantics?.parent)
              .single;
      assert(node != null);
  ...
  void assembleSemanticsNode(SemanticsNode node,
    ...
    assert(node == _semantics);
    if (semanticsAnnotator != null) semanticsAnnotator(node);
    node.addChildren(children);
    node.finalizeChildren();
  }
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder description) {
    ...
    description.add(new DiagnosticsProperty<ParentData>(
        'parentData', parentData,
    ...
    description.add(new DiagnosticsProperty<OffsetLayer>('layer', _layer,
        defaultValue: null));
    description.add(new DiagnosticsProperty<SemanticsNode>(
    ...
    description.add(new FlagProperty('isSemanticBoundary',
        value: isSemanticBoundary, ifTrue: 'semantic boundary'));
  }
...
ParentDataType extends ContainerParentDataMixin<ChildType>>
  ...
  void addAll(List<ChildType> children) {
    if (children != null) for (ChildType child in children)
      add(child);
  }
...
class FlutterErrorDetailsForRendering extends FlutterErrorDetails {
  ...
  const FlutterErrorDetailsForRendering({dynamic exception,
    StackTrace stack,
    String library,
    String context,
    this.renderObject,
