...
import 'package:flutter/ui.dart' as ui show Image, Gradient, lerpDouble;
...
class BoxShadow {
  ...
  /// and [spreadRadius].
  const BoxShadow({this.color: const Color(0xFF000000),
  ...
  static List<BoxShadow> lerpList(List<BoxShadow> a, List<BoxShadow> b,
      double t) {
    if (a == null && b == null) return null;
    a ??= <BoxShadow>[];
    b ??= <BoxShadow>[];
...
class RadialGradient extends Gradient {
  ...
  @override
  Shader createShader(Rect rect) {
    return new ui.Gradient.radial(center.withinRect(rect),
        radius * rect.shortestSide, colors, stops, tileMode);
  }
...
Iterable<Rect> _generateImageTileRects(Rect outputRect, Rect fundamentalRect,
    ImageRepeat repeat) sync* {
...
void paintImage({
  ...
  final double dx =
      (outputSize.width - destinationSize.width) * (alignment?.dx ?? 0.5);
  final double dy =
      (outputSize.height - destinationSize.height) * (alignment?.dy ?? 0.5);
  ...
  if (centerSlice == null) {
    final Rect sourceRect = (alignment ?? FractionalOffset.center)
  ...
  if (repeat != ImageRepeat.noRepeat) canvas.restore();
}
...
class DecorationImage {
  ...
  @override
  int get hashCode =>
      hashValues(image, colorFilter, fit, alignment, centerSlice, repeat);
  ...
  String toString() {
    ...
    if (fit != null &&
      ...
      properties.add('$fit');
    if (alignment != null) properties.add('$alignment');
...
class BoxDecoration extends Decoration {
  ...
  BoxDecoration lerpFrom(Decoration a, double t) {
    if (a is! BoxDecoration) return BoxDecoration.lerp(null, this, t);
    return BoxDecoration.lerp(a, this, t);
  }

  @override
  BoxDecoration lerpTo(Decoration b, double t) {
    if (b is! BoxDecoration) return BoxDecoration.lerp(this, null, t);
  ...
  BoxPainter createBoxPainter([VoidCallback onChanged]) {
    throw new UnsupportedError("Painting is not implemented in Flur");
  }
}
