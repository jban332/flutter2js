...
abstract class DiagnosticsNode {
  ...
  factory DiagnosticsNode.message(String message, {
  ...
  TextTreeConfiguration _childTextConfiguration(DiagnosticsNode child,
    ...
    for (int i = 0; i < properties.length; ++i) {
      ...
      if (property.style != DiagnosticsTreeStyle.singleLine) {
        ...
        builder.writeRaw(property.toStringDeep(
          ...
          prefixOtherLines:
          '${builder.prefixOtherLines}${propertyStyle
...
class DoubleProperty extends _NumProperty<double> {
  ...
  ///
  /// The [showName] and [level] arguments must not be null.
  DoubleProperty.lazy(String name,
...
class ObjectFlagProperty<T> extends DiagnosticsProperty<T> {
  ...
  ObjectFlagProperty(String name,
      T value, {
        this.ifPresent,
        String ifNull,
        bool showName: false,
  ...
  ObjectFlagProperty.has(String name,
      T value, {
        DiagnosticLevel level: DiagnosticLevel.info,
      })
      : ifPresent = 'has $name',
...
class DiagnosticsProperty<T> extends DiagnosticsNode {
  ...
  DiagnosticsProperty(String name,
      T value, {
        String description,
        String ifNull,
        this.ifEmpty,
  ...
  DiagnosticsProperty.lazy(String name,
      ComputePropertyValueCallback<T> computeValue, {
        String description,
        String ifNull,
        this.ifEmpty,
...
String camelCaseToHyphenatedName(String word) {
  ...
  if (word == lowerWord) return word;

  final StringBuffer buffer = new StringBuffer();
  for (int i = 0; i < word.length; i++) {
    final String lower = lowerWord[i];
    if (word[i] != lower && i > 0) buffer.write('-');
    buffer.write(lower);
  }
