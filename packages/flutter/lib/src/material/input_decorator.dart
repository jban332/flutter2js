...
class _BorderContainerState
    extends SingleTickerProviderStateMixin<_BorderContainer> {
  ...
  Widget build(BuildContext context) {
    return new CustomPaint(
      foregroundPainter: new _InputBorderPainter(
        repaint:
            new Listenable.merge(<Listenable>[_borderAnimation, widget.gap]),
...
class _HelperErrorState extends SingleTickerProviderStateMixin<_HelperError> {
...
class _RenderDecoration extends RenderBox {
  ...
  List<DiagnosticsNode> debugDescribeChildren() {
    ...
    void add(RenderBox child, String name) {
      if (child != null) value.add(input.toDiagnosticsNode(name: name));
    }
  ...
  void performLayout() {
    ...
    if (container != null) {
      ...
      final double x =
          textDirection == TextDirection.rtl ? 0.0 : _boxSize(icon).width;
    ...
    double centerLayout(RenderBox box, double x) {
      _boxParentData(box).offset =
          new Offset(x, (height - box.size.height) / 2.0);
    ...
    double baselineLayout(RenderBox box, double x) {
      _boxParentData(box).offset =
          new Offset(x, baseline - layout.boxToBaseline[box]);
      return box.size.width;
    }
      ...
      case TextDirection.rtl:
        {
          ...
          if (label != null) centerLayout(label, start - label.size.width);
          if (prefix != null)
          ...
          if (suffixIcon != null) end += centerLayout(suffixIcon, end);
      ...
      case TextDirection.ltr:
        {
          ...
          double end = right;
          if (prefixIcon != null) start += centerLayout(prefixIcon, start);
          if (label != null) centerLayout(label, start);
...
class _Decorator extends RenderObjectWidget {
  ...
  @override
  _RenderDecorationElement createElement() =>
      new _RenderDecorationElement(this);
...
class InputDecorator extends StatefulWidget {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder description) {
    ...
    description.add(new DiagnosticsProperty<TextStyle>('baseStyle', baseStyle,
        defaultValue: null));
...
class _InputDecoratorState extends TickerProviderStateMixin<InputDecorator> {
  ...
  Color _getFillColor(ThemeData themeData) {
    if (!decoration.filled) return Colors.transparent;
  ...
  bool get _hasInlineLabel =>
      !isFocused && isEmpty && decoration.labelText != null;
  ...
  TextStyle _getFloatingLabelStyle(ThemeData themeData) {
    ...
    return style.copyWith(color: color).merge(decoration.labelStyle);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final InputBorder border = decoration.border == null
        ? null
    ...
    final Widget prefix = decoration.prefixText == null
        ? null
        : new AnimatedOpacity(
            duration: _kTransitionDuration,
            curve: _kTransitionCurve,
            opacity: widget._labelIsFloating ? 1.0 : 0.0,
            child: new Text(decoration.prefixText,
                style: decoration.prefixStyle ?? inlineStyle),
          );

    final Widget suffix = decoration.suffixText == null
        ? null
        : new AnimatedOpacity(
            duration: _kTransitionDuration,
            curve: _kTransitionCurve,
            opacity: widget._labelIsFloating ? 1.0 : 0.0,
            child: new Text(decoration.suffixText,
    ...
    final Widget counter = decoration.counterText == null
        ? null
        : new Text(
            decoration.counterText,
            style: _getHelperStyle(themeData).merge(decoration.counterStyle),
            textAlign:
                textAlign == TextAlign.end ? TextAlign.start : TextAlign.end,
            overflow: TextOverflow.ellipsis,
          );
...
class InputDecoration {
  ...
  String toString() {
    ...
    if (filled) description.add('filled: true');
