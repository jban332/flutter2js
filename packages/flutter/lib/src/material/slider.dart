...
class _SliderState extends TickerProviderStateMixin<Slider> {
  _SliderState() {
...
const double _kMinimumTrackWidth =
    _kActiveThumbRadius; // biggest of the thumb radii
const double _kPreferredTotalWidth =
    _kPreferredTrackWidth + 2 * _kReactionRadius;
...
class _RenderSlider extends RenderBox {
  ...
  double _getValueFromGlobalPosition(Offset globalPosition) {
    final double visualPosition =
        (globalToLocal(globalPosition).dx - _kReactionRadius) / _trackLength;
    return _getValueFromVisualPosition(visualPosition);
  }

  double _discretize(double value) {
    double result = value.clamp(0.0, 1.0);
    if (divisions != null) result = (result * divisions).round() / divisions;
    return result;
  }
  ...
  void _handleTapUp(TapUpDetails details) {
    if (isInteractive && !_active)
      onChanged(
          _discretize(_getValueFromGlobalPosition(details.globalPosition)));
  }
  ...
  void performResize() {
    size = new Size(
      ...
      constraints.hasBoundedHeight
          ...
          : _getPreferredTotalHeight(label),
    );
  }
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    final Paint primaryPaint = new Paint()
      ..color = enabled ? _activeColor : _inactiveColor;
    ...
    final double thumbRadius = enabled
    ...
    if (enabled) {
      if (visualPosition > 0.0)
        canvas.drawRect(
            new Rect.fromLTRB(trackLeft, trackTop, trackActive, trackBottom),
            leftPaint);
      ...
      if (visualPosition > 0.0)
        canvas.drawRect(
            new Rect.fromLTRB(trackLeft, trackTop,
                trackActive - _kDisabledThumbRadius - 2, trackBottom),
            trackPaint);
      if (visualPosition < 1.0)
        canvas.drawRect(
            new Rect.fromLTRB(trackActive + _kDisabledThumbRadius + 2, trackTop,
                trackRight, trackBottom),
            trackPaint);
    }
    ...
    if (_reaction.status != AnimationStatus.dismissed) {
      ...
      if (label != null) {
        final Offset center = new Offset(
            trackActive,
            _kLabelBalloonCenterTween.evaluate(_reaction) * textScaleFactor +
                trackCenter);
        final double radius =
            _kLabelBalloonRadiusTween.evaluate(_reaction) * textScaleFactor;
        final Offset tip = new Offset(
            trackActive,
            _kLabelBalloonTipTween.evaluate(_reaction) * textScaleFactor +
                trackCenter);
        ...
        final Color reactionBaseColor =
            thumbAtMin ? _kActiveTrackColor : _activeColor;
