...
class FlexibleSpaceBar extends StatefulWidget {
  ...
  const FlexibleSpaceBar(
      {Key key, this.title, this.background, this.centerTitle})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.background != null) {
      ...
      final double opacity =
          1.0 - new Interval(fadeStart, fadeEnd).transform(t);
      ...
      if (opacity > 0.0) {
        children.add(new Positioned(
            ...
            child: new Opacity(opacity: opacity, child: widget.background)));
      }
    }
    ...
    if (widget.title != null) {
      ...
      if (opacity > 0.0) {
        ...
        final double scaleValue =
            new Tween<double>(begin: 1.5, end: 1.0).lerp(t);
        final Matrix4 scaleTransform = new Matrix4.identity()
          ..scale(scaleValue, scaleValue, 1.0);
        final Alignment titleAlignment = effectiveCenterTitle
            ? Alignment.bottomCenter
            : Alignment.bottomLeft;
        children.add(new Container(
            padding: new EdgeInsets.only(
                left: effectiveCenterTitle ? 0.0 : 72.0, bottom: 16.0),
            child: new Transform(
