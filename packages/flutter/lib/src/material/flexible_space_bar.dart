...
class _FlexibleSpaceBarState extends State<FlexibleSpaceBar> {
  ...
  Widget build(BuildContext context) {
    final _FlexibleSpaceBarSettings settings = context
        .inheritFromWidgetOfExactType(_FlexibleSpaceBarSettings);
    assert(settings !=
        null, 'A FlexibleSpaceBar must be wrapped in the widget returned by FlexibleSpaceBar.createSettings().');
    ...
    final double t = (1.0 -
        (settings.currentExtent - settings.minExtent) / (deltaExtent)).clamp(
        0.0, 1.0);
    ...
    if (widget.background != null) {
      ...
      final double opacity = 1.0 -
          new Interval(fadeStart, fadeEnd).transform(t);
      final double parallax = new Tween<double>(
          begin: 0.0, end: deltaExtent / 4.0).lerp(t);
      if (opacity > 0.0) {
    ...
    if (widget.title != null) {
      ...
      if (opacity > 0.0) {
        ...
        final double scaleValue = new Tween<double>(begin: 1.5, end: 1.0).lerp(
            t);
        final Matrix4 scaleTransform = new Matrix4.identity()
          ..scale(scaleValue, scaleValue, 1.0);
        final FractionalOffset titleAlignment = effectiveCenterTitle
            ...
            child: new Transform(
                ...
                child: new Align(
                    ...
                    child: new DefaultTextStyle(
                        style: titleStyle, child: widget.title)
                )
            )
        ));
      }
