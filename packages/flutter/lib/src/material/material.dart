...
class Material extends StatefulWidget {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder description) {
    ...
    description
        .add(new DoubleProperty('elevation', elevation, defaultValue: 0.0));
    description.add(
...
class _MaterialState extends TickerProviderStateMixin<Material> {
  ...
  Widget build(BuildContext context) {
    ...
    final BorderRadius radius =
        widget.borderRadius ?? kMaterialEdges[widget.type];
    if (contents != null) {
    ...
    if (widget.type == MaterialType.circle) {
      ...
      if (radius == null) {
        ...
        contents = new ClipRRect(borderRadius: radius, child: contents);
      }
    } else {
      contents = new AnimatedPhysicalModel(
        curve: Curves.fastOutSlowIn,
