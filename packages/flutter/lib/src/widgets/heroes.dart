...
class _HeroFlightManifest {
  ...
  Animation<double> get animation {
    return new CurvedAnimation(
      parent: (type == _HeroFlightType.push)
          ? toRoute.animation
          : fromRoute.animation,
...
class _HeroFlight {
          ...
          if (_heroOpacity.isCompleted) {
            _heroOpacity = new Tween<double>(begin: 1.0, end: 0.0)
                .chain(new CurveTween(
                    curve: new Interval(_proxyAnimation.value, 1.0)))
  ...
  void divert(_HeroFlightManifest newManifest) {
    ...
    if (manifest.type == _HeroFlightType.push &&
        newManifest.type == _HeroFlightType.pop) {
    ...
    } else if (manifest.type == _HeroFlightType.pop &&
        newManifest.type == _HeroFlightType.push) {
      ...
      if (manifest.fromHero != newManifest.toHero) {
        ...
        heroRect = _doCreateRectTween(
            heroRect.end, _globalBoundingBoxFor(newManifest.toHero.context));
      } else {
      ...
      heroRect = _doCreateRectTween(heroRect.evaluate(_proxyAnimation),
...
class HeroController extends NavigatorObserver {
  ...
  void _maybeStartHeroTransition(Route<dynamic> fromRoute,
      Route<dynamic> toRoute, _HeroFlightType flightType) {
    if (_questsEnabled &&
        toRoute != fromRoute &&
      ...
      final Animation<double> animation =
          (flightType == _HeroFlightType.push) ? to.animation : from.animation;

      // A user gesture may have already completed the pop.
      if (flightType == _HeroFlightType.pop &&
          animation.status == AnimationStatus.dismissed) return;
  ...
  void _startHeroTransition(PageRoute<dynamic> from, PageRoute<dynamic> to,
      _HeroFlightType flightType) {
    ...
    final Map<Object, _HeroState> fromHeroes =
        Hero._allHeroesFor(from.subtreeContext);
    final Map<Object, _HeroState> toHeroes =
        Hero._allHeroesFor(to.subtreeContext);
