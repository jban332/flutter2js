...
class NavigatorState extends TickerProviderStateMixin<Navigator> {
  ...
  void initState() {
    ...
    if (initialRouteName.startsWith('/') && initialRouteName.length > 1) {
      ...
      if (route == null) route = _routeNamed(Navigator.defaultRouteName);
      push(route);
    }
  ...
  void replace(
      {@required Route<dynamic> oldRoute, @required Route<dynamic> newRoute}) {
    assert(!_debugLocked);
    ...
    setState(() {
      ...
      if (index > 0) _history[index - 1].didChangeNext(newRoute);
      oldRoute.dispose();
    });
    assert(() {
      _debugLocked = false;
