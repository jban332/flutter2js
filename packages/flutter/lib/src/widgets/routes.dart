...
abstract class TransitionRoute<T> extends OverlayRoute<T> {
  ...
  new ProxyAnimation(kAlwaysDismissedAnimation);
    ...
    if (nextRoute is TransitionRoute<dynamic> &&
      ...
      if (current != null) {
        if (current is TrainHoppingAnimation) {
          ...
          new TrainHoppingAnimation(current, nextRoute._animation);
        }
      } else {
...
class _ModalScopeStatus extends InheritedWidget {
  const _ModalScopeStatus({Key key,
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder description) {
    ...
    description.add(new FlagProperty('isCurrent',
        value: isCurrent, ifTrue: 'active', ifFalse: 'inactive'));
    description
        .add(new FlagProperty('canPop', value: canPop, ifTrue: 'can pop'));
  }
}
...
abstract class ModalRoute<T> extends TransitionRoute<T>
    implements LocalHistoryRoute<T> {
  /// Creates a route that blocks interaction with previous routes.
  ModalRoute({this.settings: const RouteSettings()});
  ...
  bool get hasScopedWillPopCallback {
    return _scopeKey.currentState == null ||
        _scopeKey.currentState._willPopCallbacks.isNotEmpty;
  }
  ...
  final GlobalKey<_ModalScopeState> _scopeKey =
  new GlobalKey<_ModalScopeState>();
