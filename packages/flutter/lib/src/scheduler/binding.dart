...
abstract class SchedulerBinding extends BindingBase with ServicesBindingMixin {
    ...
    assert(() {
      if (transientCallbackCount > 0) {
        ...
        final Map<int, _FrameCallbackEntry> callbacks =
            new Map<int, _FrameCallbackEntry>.from(_transientCallbacks);
        FlutterError.reportError(new FlutterErrorDetails(
            exception: reason,
            library: 'scheduler library',
...
///   [schedulingStrategy].
abstract class SchedulerBindingMixin implements SchedulerBinding {
  static void doInitInstances(SchedulerBinding binding) {
    SchedulerBinding._instance = binding;
    ui.window.onBeginFrame = binding._handleBeginFrame;
    ui.window.onDrawFrame = binding._handleDrawFrame;
    SystemChannels.lifecycle.setMessageHandler(binding._handleLifecycleMessage);
  }

  static void doInitServiceExtensions(SchedulerBinding binding) {
    binding.registerNumericServiceExtension(
        name: 'timeDilation',
        getter: () async => timeDilation,
        setter: (double value) async {
          timeDilation = value;
  ...
  static void doUnlocked(SchedulerBinding binding) {
    if (binding._taskQueue.isNotEmpty) binding._ensureEventLoopCallback();
  }
